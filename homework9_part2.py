...
# I'd say most seem really neutral, while some are actually positive and some definitely seem negative.
...
# Logistic regression took the longest - 3.62 s. 
# 
# Random Forest took twice less, 1.52 s.
# 
# The others took less than a second:
# 
# LinearSVC - 39.3 ms
# 
# Bayes - 21.4 ms
...
# 0 should be 100% negative, and 1 100% positive, with 0.5 being neutral
...
# I really liked how logreg worked. The only time is was off in my opinion was the Kubrick sentence. In the other cases, it was more correct (in my view) than the other models. 
# The sentences the models most disagreed about:
# * The package was delivered late and the contents were broken - for some reason, Forest thought it's something good.
# * Everybody hates this movie, but I adore it -- Logreg count as positive, Forest as negative ans Bayes as neutral
# * Everybody is looking at you -- Negative for Logreg, positive for the other two. Funny, I am not sure how I feel myself about this sentence.
# * You are old enough - very negative for Logreg, quite positive for Forest.
...
# I would say we can use 0/1 when we really need only 2 shades of emotion: negative or positive, without digging into details, we can go with 0 and 1. Like if our question is smth binary like 'Do they generally like or dislike this?'
...
# I guess the answer is 'it depends'. But generally it does seem to make more sense when smth positive starts from 0.7 and higher, and negative is 0.3 and lower. 
...
# negative tweets were predicted more accurately
...
# It's better at predicting positive tweets.
...
# Maybe the fact it's quite fast?
...
# If we consider the speed as a performance factor as well, as discussed in the class, I would use Linear SVC more often. They were no huge differences, but there were some minor differences when we explored the confusion matrices.
...
# I think it's more important to be sensitive to negativity. This is what we'd like to explore in many situations of research -- hence, mislabeling the negative things as positive would hide it from us. 
...
# - feeding it more data from different sources 
...
# It suddenly feels less. But in fact this is what we should keep in ind always when talking about the 75% accuracy.
...
# I am more informed now about the inner logic of it, hence more critical towards the results.

# What would you feel comfortable using our sentiment classifier for?

# Finding leads for further investigations and manual check.
